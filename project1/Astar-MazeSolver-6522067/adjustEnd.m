...
%   outputs the adjusted调整过的 maze at the end of this function.
%
% Function Call
%   function [maze] = adjustEnd(maze, endPosition)
%
...
position = point(endPosition.row + 1, endPosition.col); %终点下面的点，并让它为路径
xTarget = endPosition.row + 1;
yTarget = endPosition.col;
maze = setMazePosition(maze, position, 1);
connection = 0;
while connection == 0
    % Search if there are any 1's in down, left, right
    % positions，如果有路径，connection=1
    ...
    elseif mazeValue(maze, position, 0, 1) == 1
        ...
        position = adjust(position, 1, 0);%如果这个点的四周都是墙（白色），则这个点下面那个点设成路径（蓝色）
        maze = setMazePosition(maze, position, 1);
    end
end
